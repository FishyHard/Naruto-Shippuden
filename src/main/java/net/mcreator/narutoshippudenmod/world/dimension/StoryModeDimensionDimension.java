...
public class StoryModeDimensionDimension extends NarutoShippudenModElements.ModElement {
	...
	public void init(FMLCommonSetupEvent event) {
		...
		replaceableBlocks.add(ForgeRegistries.BIOMES.getValue(new ResourceLocation("naruto_shippuden:story_mode_biome")).getGenerationSettings()
	...
	public void clientLoad(FMLClientSetupEvent event) {
		DimensionRenderInfo customEffect = new DimensionRenderInfo(128, true, DimensionRenderInfo.FogType.NORMAL, false, false) {
			...
			public Vector3d func_230494_a_(Vector3d color, float sunHeight) {
				return new Vector3d(0.470588235294, 0.823529411765, 0.894117647059);
			}
		...
		DeferredWorkQueue.runLater(() -> {
			try {
				...
				effectsRegistry.put(new ResourceLocation("naruto_shippuden:story_mode_dimension"), customEffect);
			} catch (Exception e) {
				e.printStackTrace();
			}
		});
